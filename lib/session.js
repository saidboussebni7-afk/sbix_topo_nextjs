...
function getSecret() {
  const s =
    ...
    'temporary-dev-secret-change-in-vercel-32chars';
...
function createAdminSession(req, res) {
  ...
  const maxAge = Number(
    process.env.ADMIN_SESSION_MAX_AGE || 60 * 60
  );
...
function clearAdminSession(res) {
  res.setHeader(
    'Set-Cookie',
    clearCookieString(ADMIN_COOKIE)
  );
}
...
function createLicenseSession(req, res, data = {}) {
  ...
  const maxAge = Number(
    process.env.LICENSE_SESSION_MAX_AGE ||
      12 * 60 * 60
  );
...
function clearLicenseSession(res) {
  res.setHeader(
    'Set-Cookie',
    clearCookieString(LICENSE_COOKIE)
  );
}
